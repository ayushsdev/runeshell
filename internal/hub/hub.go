...
var testHookBeforeAddClient func()
...
func (c *clientState) writeJSON(v any) error {
	c.writeMu.Lock()
	defer c.writeMu.Unlock()
	return c.conn.WriteJSON(v)
}

func (c *clientState) writeMessage(messageType int, data []byte) error {
	c.writeMu.Lock()
	defer c.writeMu.Unlock()
		...
		case websocket.BinaryMessage:
			...
			for _, c := range clients {
				if c.proto < 2 {
					...
					_ = c.client.writeMessage(websocket.BinaryMessage, payload)
					continue
				}
				_ = c.client.writeMessage(websocket.BinaryMessage, data)
			}
...
func (h *Hub) ServeClientConn(conn WSConn, claims Claims) {
	...
	if testHookBeforeAddClient != nil {
		testHookBeforeAddClient()
	}
	...
	if err := client.writeJSON(ControlMessage{Type: "attached", Write: claims.Write, Status: "ok"}); err != nil {
		return
	}
	h.sendWriteStatus(client, h.isWriter(claims.AgentID, clientID))
	...
	for {
		...
		case websocket.TextMessage:
			var msg ControlMessage
			if err := json.Unmarshal(data, &msg); err != nil {
				_ = client.writeJSON(ControlMessage{Type: "error", Code: "bad_request", Message: "invalid json"})
				return
			}
			switch msg.Type {
			case "attach":
				...
				if msg.SessionID == "" {
					_ = client.writeJSON(ControlMessage{Type: "error", Code: "bad_request", Message: "session_id required"})
					return
				}
				if claims.SessionID != "" && msg.SessionID != claims.SessionID {
					_ = client.writeJSON(ControlMessage{Type: "error", Code: "not_authorized", Message: "session mismatch"})
					return
				}
			...
			case "request_write":
				if !claims.Write {
					_ = client.writeJSON(ControlMessage{Type: "write_denied", Code: "not_authorized", Message: "not authorized for write"})
					break
				}
				if !h.webWritable(claims.AgentID) {
					_ = client.writeJSON(ControlMessage{Type: "write_denied", Code: "locked", Message: "web input locked"})
					break
				}
				if h.hasWriter(claims.AgentID) && !h.isWriter(claims.AgentID, clientID) {
					_ = client.writeJSON(ControlMessage{Type: "write_denied", Code: "another_writer", Message: "another writer active"})
					break
				}
		...
		case websocket.BinaryMessage:
			...
			frame, _ := muxframe.Encode(sid, payload)
			_ = agent.writeMessage(websocket.BinaryMessage, frame)
		}
	}
}
...
func (h *Hub) sendWriteStatus(client *clientState, write bool) {
	_ = client.writeJSON(ControlMessage{Type: "write_status", Write: write})
}
...
func (h *Hub) broadcastWriteStatus(agentID string) {
	...
	h.mu.Unlock()
	for _, c := range clients {
		_ = c.client.writeJSON(ControlMessage{Type: "write_status", Write: c.id == writer})
	}
}
